...
*                                        PROJECT PASSPORT                                          *
****************************************************************************************************
Title: Opposing Troops
Description: The program deploys armor stand entities in the form of two opposing forces.

Programming Language: JavaScript (Static TypeScript)
Version: 1.0
Creation Date: March 2026
Author: Dmytro Ivanenko, dmytro.ivanenko@gmail.com
Publication: https://github.com/DmytroIvanenko/minecraft-troops-deployment

Technical Features:
The program automates the placement of armor stands to represent two forces positioned 
facing each other.
Code commands "equip" each force with distinct armor sets and "place" weapons 
...
function attackers() {
    ...
    for (let eastCounter = 1; eastCounter <= 2; eastCounter++) {
        ...
        for (let southCounter = 1; southCounter <= 30; southCounter++) {
            ...
            player.execute(`/summon armor_stand "§4Attacker" ~${xDistance} ~0 ~${zDistance}`);
            player.execute(`/tp @e[type=armor_stand, c=1, x=~${xDistance}, y=~0, z=~${zDistance}] ~${xDistance} ~0 ~${zDistance} facing ~0 ~0 ~${zDistance}`);

            let searchMarker = `@e[type=armor_stand, c=1, x=~${xDistance}, y=~0, z=~${zDistance}]`;

            // 1.1.2. Equip: chest, legs, feet
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.chest 0 diamond_chestplate 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.legs 0 diamond_leggings 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.feet 0 diamond_boots 1 0`);

            // 1.1.3. Head (helmet), weapon and shield in hands
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.head 0 skull 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.weapon.mainhand 0 trident 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.weapon.offhand 0 shield 1 0`);
    ...
    let searchMarker = `@e[type=armor_stand, c=1, x=~-6, y=~0, z=~0]`;
...
function defenders() {
    ...
    for (let southCounter = 1; southCounter <= 30; southCounter++) {
        ...
        player.execute(`/summon armor_stand "§7Lancer" ~10 ~0 ~${zDistance}`);
        player.execute(`/tp @e[type=armor_stand, c=1, x=~10, y=~0, z=~${zDistance}] ~10 ~0 ~${zDistance} facing ~0 ~0 ~${zDistance}`);

        let searchMarker = `@e[type=armor_stand, c=1, x=~10, y=~0, z=~${zDistance}]`;

        // 2.1.2. Equip: chest, legs, feet
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.chest 0 netherite_chestplate 1 0`);
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.legs 0 netherite_leggings 1 0`);
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.feet 0 netherite_boots 1 0`);

        // 2.1.3. Head (helmet), weapon and shield in hands
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.head 0 skull 1 1`);
        player.execute(`/replaceitem entity ${searchMarker} slot.weapon.mainhand 0 netherite_spear 1 0`);
        player.execute(`/replaceitem entity ${searchMarker} slot.weapon.offhand 0 shield 1 0`);
    ...
    for (let eastCounter = 1; eastCounter <= 2; eastCounter++) {
        ...
        for (let southCounter = 1; southCounter <= 30; southCounter++) {
            ...
            player.execute(`/summon armor_stand "§6Swordsman" ~${xDistance} ~0 ~${zDistance}`);
            player.execute(`/tp @e[type=armor_stand, c=1, x=~${xDistance}, y=~0, z=~${zDistance}] ~${xDistance} ~0 ~${zDistance} facing ~0 ~0 ~${zDistance}`);

            let searchMarker = `@e[type=armor_stand, c=1, x=~${xDistance}, y=~0, z=~${zDistance}]`;

            // 2.2.2. Equip: chest, legs, feet
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.chest 0 iron_chestplate 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.legs 0 iron_leggings 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.feet 0 iron_boots 1 0`);

            // 2.2.3. Head (helmet), weapon and shield in hands
            player.execute(`/replaceitem entity ${searchMarker} slot.armor.head 0 skull 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.weapon.mainhand 0 netherite_sword 1 0`);
            player.execute(`/replaceitem entity ${searchMarker} slot.weapon.offhand 0 shield 1 0`);
    ...
    for (let southCounter = 1; southCounter <= 30; southCounter++) {
        ...
        player.execute(`/summon armor_stand "§2Archer" 17 ~0 ~${zDistance}`);
        player.execute(`/tp @e[type=armor_stand, c=1, x=~17, y=~0, z=~${zDistance}] ~17 ~0 ~${zDistance} facing ~0 ~0 ~${zDistance}`);

        let searchMarker = `@e[type=armor_stand, c=1, x=~17, y=~0, z=~${zDistance}]`;

        // 2.3.2. Equip: chest, legs, feet
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.chest 0 leather_chestplate 1 0`);
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.legs 0 leather_leggings 1 0`);
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.feet 0 iron_boots 1 0`);

        // 2.3.3. Head (helmet), weapon and shield in hands
        player.execute(`/replaceitem entity ${searchMarker} slot.armor.head 0 skull 1 3`);
        player.execute(`/replaceitem entity ${searchMarker} slot.weapon.mainhand 0 bow 1 0`);
